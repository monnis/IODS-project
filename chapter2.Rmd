...
For the model parameters: As I said above "the results imply that one point of the attitude metric predicts roughly 0.35 points in the exam. The intercept is ~11.6, meaning that this is the model's baseline points for everyone - points that even a person with an attitude score of zero can get. However, this is a hypothetical case, as the minimum of attitude points in the sample is 14 points". Removing the nonsignificant independent variables does not change the results much.

The R squared gains the value of 0.1906. This is the percentage  of the variation that the linear model explains out of the total variation (or 19.06 is to be specific) and means that the actual data points are not that close to the regression line. In other words: we compare a "baseline model" and our fitted model. The baseline model is a totally useless model which always predicts the mean of observed dependent variable and does not use the independent variables at all. If the Fitted model is close to the baseline model (the mean of the dependent variable), the R squared is small. However, if the predicted values and the actual observations are closer together, the R squared is big. Theoretically, an R squared of one would mean that the data points would overlap the regression line perfectly.
...
Finally, the leverage plot can help identify outliers' impact on the model parameters. Outliers are problematic as they often don't represent the sample, and can deviate the regression line. Here, no outliers really stand out, which is great!  

In addition to tools like these, common sense also often helps. 
